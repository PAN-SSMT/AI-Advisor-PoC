...
const CurrentDocuments: React.FC = () => {
  return (
    <div className="p-6 md:p-8">
        <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500 mb-6">Files currently indexed and available to the AI Advisor.</p>
        <div className="bg-white overflow-hidden sm:rounded-md">
            <ul role="list" className="divide-y divide-gray-200">
                {docs.map(doc => (
                    <li key={doc.name} className="px-4 py-4 sm:px-6 hover:bg-gray-50 flex items-center justify-between transition duration-150 ease-in-out cursor-pointer">
                        <div className="flex items-center min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                                ...
                                <div className="hidden md:block">
                                    <div>
                                        <p className="text-sm text-gray-900">Uploaded on <time dateTime={doc.date}>{doc.date}</time></p>
                                    </div>
                                </div>
                            </div>
                        </div>
