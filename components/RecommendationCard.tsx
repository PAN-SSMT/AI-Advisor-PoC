      ...
      {!compact && (
        <div className="flex justify-between items-start mt-2">
          <div className="w-[70%] pr-4">
              ...
              <details>
                <summary className="text-sm font-medium text-blue-600 cursor-pointer hover:underline">Show Implementation Instructions</summary>
          ...
          <div className="flex items-center justify-end gap-3">
            {recommendation.status === RecommendationStatus.Approved && (
              <span className="text-sm font-semibold text-green-600 flex items-center"><CheckIcon className="w-5 h-5 mr-1"/> Added to Approved Actions List</span>
            )}
            ...
            {isActionable && !isReadOnly && (
              <>
                ...
                <button
                  onClick={() => onUpdateStatus(recommendation.id, RecommendationStatus.Approved)}
                  className="px-4 py-3 text-xs font-medium text-green-800 bg-green-100 border border-green-800 rounded-md shadow-sm hover:bg-green-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 text-center leading-tight"
                >
                  <span className="whitespace-nowrap">Add to Approved</span>
                  <br />
